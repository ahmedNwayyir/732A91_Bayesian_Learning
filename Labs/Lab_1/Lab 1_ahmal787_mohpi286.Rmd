...
knitr::opts_chunk$set(echo = FALSE, fig.align = "center", warning = FALSE, out.width = "80%", fig.height=4)
...
\textcolor{mycolor}{Expected Value of Mean for the Beta Distribution}
$$\color{mycolor}{E[X] = \frac{\alpha}{\alpha + \beta}}$$

\textcolor{mycolor}{And expected value of Standard Deviation:}
$$\color{mycolor}{sd[X] = \sqrt{\frac{\alpha \cdot \beta}{(\alpha + \beta)^2 \cdot (\alpha + \beta + 1)}}}$$
```{r 1_a}
```
...
\textcolor{mycolor}{We draw a 10,000 sample from the posterior distribution and calculate the average of how many instances have probability above 0.3 in the whole sample and compare it with the 1-0.3 area}
...
```{r echo=FALSE, fig.cap="The wind direction data. Angles are measured clock-wise starting from North.", fig.pos="H", dpi=100}
library(knitr)
include_graphics("other/wind.png")
```
